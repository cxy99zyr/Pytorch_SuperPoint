...
""" Data augmentation of 2D images with keypoint handling """
...
def dummy(image, keypoints):
    """Dummy function that returns the inputs unchanged."""
...
def resize_after_crop(orig_img, cropped_img, keypoints, random_state=None):
    ...
    if (cropped_img.shape[1] * ratio) > cropped_img.shape[0]:  # columns too long
        ...
        if isinstance(keypoints, torch.Tensor):
            ...
            new_keypoints = keypoints[mask].float()
    ...
    if isinstance(new_keypoints, torch.Tensor):
...
def crop_after_transform(orig_img, warped_img, transform,
    ...
    Args:
        ...
        orig_keypoints: Original keypoints.
        keypoints: Transformed keypoints.
        random_state: Random state for reproducibility.
        
    Returns:
        Tuple of (cropped_image, adjusted_keypoints).
    """
...
def affine_transform(img, keypoints, random_state=None, affine_params=(0.05, 0.15)):
    ...
    Returns:
        Tuple of (transformed_image, transformed_keypoints).
    """
...
def elastic_transform(img, keypoints, random_state=None,
    ...
    Args:
        ...
        keypoints: Nx2 array of keypoints.
        random_state: Random state for reproducibility.
        sigma_params: Parameters for Gaussian filter sigma.
        alpha_params: Parameters for distortion intensity.
        padding: Padding to remove strange artifacts.
    ...
    for i in range(len(keypoints)):
        kp = keypoints[i]
        # Find the closest pixel in the inverse map
        closest_x = int(round(kp[0]))
        closest_y = int(round(kp[1]))
        if (closest_x >= 0 and closest_x < shape[1] and
            closest_y >= 0 and closest_y < shape[0]):
            new_keypoints[i] = [inverse_map_x[closest_y, closest_x],
                              inverse_map_y[closest_y, closest_x]]

    # Remove padding
    distorted_img = distorted_img[padding:-padding, padding:-padding]
    new_keypoints -= padding
    new_keypoints = keep_points_inside(new_keypoints,
                                     (shape[0]-2*padding, shape[1]-2*padding))

    # Convert back to tensor if input was tensor
    if isinstance(img, torch.Tensor):
        distorted_img = torch.from_numpy(distorted_img)
        new_keypoints = torch.from_numpy(new_keypoints)

    return distorted_img, new_keypoints
...
def random_crop(img, keypoints, random_state=None, min_crop_ratio=0.5):
    ...
    Args:
        ...
        keypoints: Nx2 array of keypoints.
        random_state: Random state for reproducibility.
        min_crop_ratio: Minimum ratio of the cropped image size to original size.
        
    Returns:
        Tuple of (cropped_image, cropped_keypoints).
    """
    if random_state is None:
        random_state = np.random.RandomState(None)
    ...
    shape = img.shape
    ratio = random_state.uniform(min_crop_ratio, 1.0)
    new_size = [int(shape[0] * ratio), int(shape[1] * ratio)]
    
    # Random crop position
    start_row = random_state.randint(0, shape[0] - new_size[0] + 1)
    start_col = random_state.randint(0, shape[1] - new_size[1] + 1)
    
    cropped_img = img[start_row:start_row+new_size[0],
                     start_col:start_col+new_size[1]]

    # Adjust keypoints
    new_keypoints = np.copy(keypoints)
    new_keypoints[:, 0] -= start_col
    new_keypoints[:, 1] -= start_row
    new_keypoints = keep_points_inside(new_keypoints, new_size)
    ...
    return cropped_img, new_keypoints 
...
def additive_gaussian_noise(img, keypoints, random_state=None, std=(5, 95)):
    ...
    Returns:
        Tuple of (noisy_image, keypoints).
    """
    if random_state is None:
        random_state = np.random.RandomState(None)
...
def random_brightness(img, keypoints, random_state=None, max_change=50):
    ...
    Returns:
        Tuple of (adjusted_image, keypoints).
    """
    if random_state is None:
        random_state = np.random.RandomState(None)
...
def add_shade(img, keypoints, random_state=None, nb_ellipses=20,
    ...
    """Add shade to the image by overlaying several dark ellipses.
    
    Args:
        img: Input image.
        keypoints: Nx2 array of keypoints.
        random_state: Random state for reproducibility.
        nb_ellipses: Number of ellipses to overlay.
        amplitude: Range for the random amplitude of each ellipse.
        kernel_size_interval: Range for the size of the ellipses.
        
    Returns:
        Tuple of (shaded_image, keypoints).
    """
    if random_state is None:
        random_state = np.random.RandomState(None)
    ...
    for _ in range(nb_ellipses):
        ...
        cv.ellipse(mask, (x, y), (ax, ay), angle, 0, 360, 1, -1)

    kernel_size = random_state.randint(kernel_size_interval[0],
                                     kernel_size_interval[1])
    if kernel_size % 2 == 0:  # kernel_size has to be odd
        kernel_size += 1
    mask = cv.GaussianBlur(mask, (kernel_size, kernel_size), 0)
    mask = mask / np.max(mask)
    alpha = random_state.rand()*(amplitude[1]-amplitude[0]) + amplitude[0]
    shaded = img * (1 - alpha*mask[..., np.newaxis])
    ...
    return shaded, keypoints


def motion_blur(img, keypoints, max_ksize=10):
    """Apply motion blur to the image with a random kernel size.
    
    Args:
        img: Input image.
        keypoints: Nx2 array of keypoints.
        max_ksize: Maximum kernel size for motion blur.
        
    Returns:
        Tuple of (blurred_image, keypoints).
    """
    # Convert to numpy if needed
    is_tensor = isinstance(img, torch.Tensor)
    if is_tensor:
    ...
    ksize = np.random.randint(3, max_ksize)
    if ksize % 2 == 0:
        ksize += 1
...
def add_fog(img, keypoints, random_state=None, max_nb_ellipses=20,
    ...
    Returns:
        Tuple of (foggy_image, keypoints).
    """
    if random_state is None:
        random_state = np.random.RandomState(None)
    ...
    for i in range(max_nb_ellipses):
        ...
        # Add the ellipse with random color and angle
    ...
    # Convert back to tensor if input was tensor
    if is_tensor:
        shaded_img = torch.from_numpy(shaded_img)
