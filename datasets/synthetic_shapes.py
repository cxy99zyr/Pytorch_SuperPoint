...
class SyntheticShapes(BaseDataset):
    ...
    def dump_primitive_data(self, primitive, tar_path, config):
        ...
        print('下载数据到哪：', temp_dir)
        print(f'\n开始生成 {primitive} 的合成数据...')
        synthetic_dataset.set_random_state(np.random.RandomState(
            config['generation']['random_seed']))
        
        # 使用tqdm显示总体进度
        total_samples = sum(self.config['generation']['split_sizes'].values())
        print('总计多少样本',total_samples)
        with tqdm(total=total_samples, desc=f"生成 {primitive} 数据", position=0, leave=False, ncols=80, mininterval=1.0) as pbar:
    ...
    def _init_dataset(self, **config):
        ...
        print("\n开始生成数据集...")
        # 使用tqdm显示总体进度
        for primitive in tqdm(primitives, desc="总体进度", position=0, leave=True):
            ...
            for member in tqdm(members, desc=f"解压 {primitive}", position=0, leave=False, ncols=80, mininterval=1.0):
            ...
            print(f'处理 {primitive} 的文件...')
            for s in tqdm(splits.keys(), desc=f"处理分割", position=0, leave=False, ncols=80):
                e = [str(p) for p in Path(path, 'images', s).iterdir()]
                f = [p.replace('images', 'points') for p in e]
                f = [p.replace('.png', '.npy') for p in f]
                total_files = int(truncate*len(e))
                splits[s]['images'].extend(e[:total_files])
                splits[s]['points'].extend(f[:total_files])
                print(f'  - {s}: 添加了 {total_files} 个文件')

        print("\n打乱数据集...")
        # Shuffle with progress bar
        for s in tqdm(splits, desc="打乱数据", position=0, leave=False, ncols=80):
        ...
        # Store the file paths and print summary
        self.splits = splits
        self.size = {s: len(splits[s]['images']) for s in splits}
        
        print("\n数据集生成完成！")
        print("数据集大小统计：")
        for split_name, split_size in self.size.items():
            print(f"- {split_name}: {split_size} 个样本")

        # 检查并使用配置中的split_sizes
        if 'generation' in config and 'split_sizes' in config['generation']:
            self.config['generation']['split_sizes'] = config['generation']['split_sizes']
        # 使用默认值
        else:
            self.config['generation']['split_sizes'] = {
                'training': 100,
                'validation': 20,
                'test': 50
            }
